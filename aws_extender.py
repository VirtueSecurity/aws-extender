...
class BurpHTTPConnection(object):
	def __init__(self, host, port=80, strict=False, timeout=20, source_address=None):
		self.host = host
		self.port = port
		self.strict = strict
...
class BurpHTTPResponse:
	def __init__(self, bytes):
		...
		self.msg = Message(StringIO(self.strHeaders), True)
...
class BurpExtender(IBurpExtender, IScannerCheck, ITab):
	...
	def __init__(self):
		self.ext_name = 'AWS Extender MOD'
		self.callbacks = None
	...
	def __init__(self, request_response, callbacks, opts):
		...
		try:
			...
			if not (self.aws_access_key and self.aws_secret_key):
				...
				self.boto_s3_con = S3Connection(anon=True, https_connection_factory=httpsConnectionfactoryParam,calling_format=OrdinaryCallingFormat())

			if not (self.gs_access_key and self.gs_secret_key):
				self.boto_gs_con = S3Connection(anon=True, host='storage.googleapis.com', https_connection_factory=httpsConnectionfactoryParam,calling_format=OrdinaryCallingFormat())
		except NameError:
			pass
